...
describe('Sorted Dictionary', () => {
  ...
  it('instantiate dictionary from json', () => {
    let jsonKeyValuePairArray = '[{"key": 2, "value": {"id":2, "name": "Ford", "model": "F 100", "country": "US", "isLuxury": true }},{"key": 1, "value": {"id":1, "name": "Mercedez", "model": "S 400", "country": "Germany", "isLuxury": true }}]';
    
    let keyValueArray: KeyValuePair<number, Car>[] = JSON.parse(jsonKeyValuePairArray);

    let list = new SortedDictionary<number, Car>(null, keyValueArray);

    var luxuryCars = list.where(x => x.value.isLuxury);

    expect(list.length == 2);
    expect(luxuryCars.toArray()[0].value.name == "Mercedez");
    expect(luxuryCars.toArray()[0].value.name == "Ford");
  });
